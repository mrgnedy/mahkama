...
class ColorsD {
  static Color main = Color.fromARGB(255, 12, 126, 93);
...
class StylesD {
  static Widget loadImage(
      double height, double width, ImageChunkEvent chunk, Widget child) {
    return chunk == null
        ? child
        : LiquidLinearProgressIndicator(
            ...
            center: Container(
              ...
              decoration: BoxDecoration(
                  color: ColorsD.main, borderRadius: BorderRadius.circular(12)),
              child: Center(
                child: Text(
...
class BuildAppBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  BuildAppBar({Key key, this.height, this.callback, this.title, this.image})
      : super(key: key);
  @override
  Widget build(BuildContext context) {
    final size = MediaQuery.of(context).size;
    return Row(
      ...
      children: <Widget>[
        ...
        Txt(title, style: txtSyle),
        InkWell(
          onTap: callback ?? () => ExtendedNavigator.rootNavigator.pop(),
          child: Container(
            // color: Colors.yellow,
            alignment: Alignment.center,
            width: size.width / 5,
            child: Image.asset(image ?? R.ASSETS_IMAGES_BACK_PNG),
          ),
        )
      ],
    );
